...
export function useChatState() {
   ...
   /** Local session store: list metadata + loaded/streamed messages */
   ...
   const chatDetailQuery = useQuery({
      queryKey: queryKeys.chats.detail(activeId),
      queryFn: () => fetchChatDetail(activeId),
      enabled: activeId !== DRAFT_ID,
   })
   ...
   const loadingMessages =
      ...
      !sessions.find((s) => s.id === activeId)?.messagesLoaded
   const nextCursor = chatsQuery.data?.pages.at(-1)?.nextCursor ?? null
   const hasMore = Boolean(chatsQuery.hasNextPage)

   // Sync chat list pages → local sessions (preserve loaded messages)
   useEffect(() => {
      ...
      const chats = chatsQuery.data.pages.flatMap((p) => p.chats).map(mapListItem)
      setSessions((prev) => {
         ...
         // Keep sessions created client-side (e.g. mid-stream) until the list includes them
         const localOnly = prev.filter(
            (s) =>
               s.id !== DRAFT_ID &&
               !serverIds.has(s.id) &&
               (s.messagesLoaded || s.messages.length > 0)
         )
         const draftSession = prev.find((s) => s.id === DRAFT_ID)
         const withDraft =
            draftSession && draftSession.messages.length > 0
               ? draftSession
               : emptyDraft()
         return [withDraft, ...localOnly, ...merged]
      })
      setActiveId((current) => {
         if (current === DRAFT_ID) return DRAFT_ID
         // Don't kick the user off a chat that exists only locally yet
         return current
      })
   }, [chatsQuery.data, chatsQuery.isError])

   // Sync chat detail → local sessions
   useEffect(() => {
      if (!chatDetailQuery.data || activeId === DRAFT_ID) return
      if (chatDetailQuery.isError) {
         setError(
            getErrorMessage(chatDetailQuery.error, "Failed to load chat messages")
         )
         return
      }

      const chat = chatDetailQuery.data
      // Don't clobber in-flight stream
      if (sending) return
      ...
      const messages: Message[] = (chat.messages ?? []).map((m) => ({
         ...
         content: m.content,
         sourceChunks:
            (m.sourceChunks as Message["sourceChunks"] | null | undefined) ??
            undefined,
         createdAt:
   ...
   }, [chatDetailQuery.data, chatDetailQuery.isError, chatDetailQuery.error, activeId, sending])
   ...
   useEffect(() => {
      ...
      const observer = new IntersectionObserver(
         (entries) => {
            if (entries[0]?.isIntersecting && chatsQuery.hasNextPage && !chatsQuery.isFetchingNextPage) {
   ...
   }, [chatsQuery.hasNextPage, chatsQuery.isFetchingNextPage, chatsQuery.fetchNextPage, sessions.length])
   ...
   const deleteChatMutation = useMutation({
      ...
      onError: (_e, _id, ctx) => {
         if (ctx?.snapshot) setSessions(ctx.snapshot)
   ...
   const moveToProjectMutation = useMutation({
      ...
      onMutate: async ({ chatId, projectId, projectName }) => {
         setSessions((prev) =>
            prev.map((s) =>
   ...
   const saveProjectMutation = useMutation({
      ...
      }) => apiUpdateProject(id, { name, systemPrompt }),
      onSuccess: (updated) => {
         setSessions((prev) =>
            prev.map((s) =>
   ...
   const togglePin = async (id: string) => {
      ...
      if (!session) return
      pinMutation.mutate({ id, pinned: !session.pinned })
   }
   ...
   const moveToProject = async (chatId: string, projectId: string | null) => {
      ...
      moveToProjectMutation.mutate({ chatId, projectId, projectName })
   }
   ...
   const saveProject = async () => {
      if (!editingProject || saveProjectMutation.isPending) return
   ...
   const setTurnVersion = (userMessageId: string, nextIndex: number) => {
      ...
      setSessions((prev) =>
         prev.map((s) => {
            ...
            if (!versions?.length || nextIndex < 0 || nextIndex >= versions.length)
               return s
   ...
   const sendMessage = async () => {
      ...
      pendingEditVersionsRef.current =
         priorVersions.length > 0 ? priorVersions : null
      ...
      try {
         ...
         const applyDone = (event: Extract<StreamEvent, { type: "done" }>) => {
            ...
            const pVersions = pendingEditVersionsRef.current
            const allVersions =
               pVersions && pVersions.length > 0
                  ? [...pVersions, newVersion]
                  : undefined
            ...
            // Keep TanStack Query cache in sync after stream completes
            void queryClient.invalidateQueries({ queryKey: queryKeys.chats.list() })
            void queryClient.setQueryData(queryKeys.chats.detail(chatId), {
               id: chatId,
               title: event.title,
               pinned: false,
               projectId: null,
            ...
            // Prefer full refetch of detail so history is correct for multi-turn
   ...
   return {
      ...
      editProjectName,
      editProjectPrompt,
      savingProject: saveProjectMutation.isPending,
      openPanel,
      panelProjectIds,
      openSourceMsgId,
      ...
      deleteTarget,
      deleting:
         deleteChatMutation.isPending || deleteProjectMutation.isPending,
      // Refs
      textareaRef,
      fileRef,
      bottomRef,
